...
void	_rebase_objects(t_scene *scene)
{
	...
	{
		_rebase_coord(scene->plane[k]->r_pos, scene->plane[k]->pos, scene->camera->vect);
		_rebase_coord(scene->plane[k]->r_vect, scene->plane[k]->vect, scene->camera->vect);

	}
	k = -1;
	while (scene->sphere && scene->sphere[++k])
		_rebase_coord(scene->sphere[k]->r_pos, scene->sphere[k]->pos, scene->camera->vect);
	k = -1;
